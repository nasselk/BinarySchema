import { type Schema, type EncodedData, FieldType } from "../types.js";
...
export function compileEncoder<T extends Schema>(schema: T, bitLength: number): encoder<T> {
	...
	const needsDynamicSizing = Object.values(schema.fields).some((field) => field.dependencies?.length || (field as any).optional || field.list || field.type === FieldType.Buffer || field.type === FieldType.String);
	...
	if (needsDynamicSizing) {
		...
		for (const [name, field] of Object.entries(schema.fields)) {
			if (field.dependencies?.length || (field as any).optional || field.list || field.type === FieldType.Buffer || field.type === FieldType.String) {
				if (field.list) {
					body += `
				...
				body += `
				if (${(field as any).default !== undefined && !(field as any).optional ? true : `item${name} !== undefined`}
			`;
			...
			`;
				...
				switch (field.type) {
					...
					case FieldType.Buffer: {
						if (field.includeSize && (field.list || (field as any).optional || field.dependencies?.length)) {
							body += `
							bitLength += 16;
						`;
						}
	...
	function encodeFunction(data: EncodedData<T>, writer?: BufferWriter): Uint8Array | number {
